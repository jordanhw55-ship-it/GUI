...
class BorderManager:
    ...
    def __init__(self, app):
        ...
        self.border_presets = {
            ...
            "HP Frame": { # Spans from humanuitile01 to humanuitile02
                "target_tile": "humanuitile01", "shape_type": "relative_rect", "shape_data": [0.8398, 0.8887, 0.255859375, 0.046875]
            },
            "Mana Frame": {
    ...
    def _render_border_image(self, logical_size, render_size):
        ...
        if growth_direction == 'in':
            ...
            draw.rectangle([0, 0, logical_w - 1, logical_h - 1], fill=255)
            # The inner cutout is inset by the thickness. The second point is exclusive, so we must subtract 1 from the calculated coordinate.
            draw.rectangle([thickness, thickness, logical_w - thickness - 1, logical_h - thickness - 1], fill=0)
        else: # 'out'
            # The outer shape is the full render size. The second point is exclusive.
            draw.rectangle([0, 0, render_w - 1, render_h - 1], fill=255)
